...
if __name__ == "__main__":
    ...
    heterogeneous_map = HeterogeneousMap( # Problem with sampling at points...
        x=np.array([-11000, 30000, -11000, 30000]),
        y=np.array([-11000, -11000, 30000, 30000]),
        speed_multipliers=np.tile(
            np.array([[0.9, 1.2, 0.9, 1.2]]),
            (len(wind_directions)*len(wind_speeds), 1),
        ),
        wind_directions=np.tile(wind_directions, len(wind_speeds)),
        wind_speeds=np.repeat(wind_speeds, len(wind_directions)),
    )
    ...
    if save_figs:
        fig.savefig(
            "figs/candidate_power_map_extonly.png", dpi=300, bbox_inches="tight", format="png"
        )
    ...
    if save_figs:
        fig.savefig(
            "figs/existing_power_map_extonly.png", dpi=300, bbox_inches="tight", format="png"
        )
    ...
    # if save_figs:
    #     fig.savefig(
    #         "figs/subset_power_map_extonly.png", dpi=300, bbox_inches="tight", format="png"
    #     )
